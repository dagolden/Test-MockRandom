...
is (oneish(), (2**32 - 1)/(2**32), 
    'is oneish nearly one');
is (rand(), 0, 
...
srand();
is (rand(), 0, 
    'testing srand() gives rand() == 0');

srand(oneish);
is (rand(), oneish, 
    'testing srand(oneish) gives rand == oneish');

srand(.5);
is (rand(), .5, 
    'testing srand(.5) gives rand == .5');

srand(0);
is (rand(), 0, 
    'testing srand(0) gives rand == 0');
...
is (rand(), 0, 
    'testing rand == 0 (nothing left in srand array');
